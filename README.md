...
4. **WordPress digest — NOT optional, NOT covered by step 3.** WordPress is a 6th, separate deliverable per day, not another pillar's platform target — `publish-all.mjs` has zero WordPress awareness, so publishing all 5 video pillars does NOT publish the digest. After drafting `public/wordpress/<date>.json` and passing `check-duplicate.mjs`/`check-staleness.mjs` (a DUPLICATE verdict against that same day's own 5 pillars is the expected self-reference false positive — proceed), it still has to be explicitly sent: `node pipeline/wordpress-cover.mjs <date>` then `node pipeline/wordpress-publish.mjs <date>`. A drafted-but-unpublished digest looks identical to a real one in every check that only looks for the FILE — see the note on `wordpressPostId` below. Real incident: 2026-09-22's digest was drafted, passed both checks, and sat unpublished for a full day before the user caught it directly on the live site — the two coverage checkers reported "clean" the entire time. **A digest is only actually done once `public/wordpress/<date>.json` has a real `wordpressPostId` field, written back by `wordpress-publish.mjs` after a successful send** — its mere presence on disk proves nothing.
5. **Bluesky** fires via `.github/workflows/bluesky-schedule.yml`, a cron-triggered GitHub Action that calls `bluesky-slot-runner.mjs` at each slot's actual instant — this is a workaround for Bluesky's API having no "publish later."
6. **Facebook first comment** fires via `.github/workflows/facebook-first-comment.yml` shortly after each slot, posting an auto-generated engagement question once the post is actually live (not at schedule time — see that workflow's header for why).
7. **Homepage sync** fires via `.github/workflows/homepage-sync.yml` shortly after each slot, rebuilding bharatat100.com's archive from whatever's now live and pushing — Netlify is Git-linked to this repo and auto-deploys on push.

Steps 5-7 all run independently on cron, all push to `main`, and all `git pull --rebase` before pushing to avoid stepping on each other (see comments in each workflow).

`check-missed-days.mjs` and `check-platform-coverage.mjs` both check for a real `wordpressPostId` (not just file existence) as of 2026-09-23 — a repeat of the Sept 22 gap should now surface on the very next routine run instead of going unnoticed.
